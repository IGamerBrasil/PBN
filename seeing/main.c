...
void processa()
{
    ...
    Pixel(*in)[width] = (Pixel(*)[width])pic[0].img;//nao mexer
    Pixel(*out)[width] = (Pixel(*)[width])pic[1].img;//mexer
    ...
    for(int y=0; y<height; y++) {
        for(int x=0; x<width; x++) {
            int lum = (in[y][x].r*0.3 + in[y][x].g*0.59 + in[y][x].b*0.11); // luminancia = contraste
